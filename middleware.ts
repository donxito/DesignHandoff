...
export async function middleware(req: NextRequest) {
  ...
  try {
    ...
    res.headers.set('x-middleware-auth-status', data.session ? 'authenticated' : 'unauthenticated');
    ...
    if (isAuthRoute && data.session) {
      console.log('Middleware: Authenticated access to auth route, redirecting to dashboard');
